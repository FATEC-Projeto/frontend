...
export default function FirstAccessPage() {
  ...
  useEffect(() => {
    const fromQuery = q.get("uid");
    const fromLS =
      typeof window !== "undefined" &&
      (localStorage.getItem("firstAccessUserId") || localStorage.getItem("userId"));
    setUserId(fromQuery || fromLS || null);
  }, [q]);
  ...
  const validEmail =
    !personalEmail ||
    /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(personalEmail); // opcional, mas se preencher, valida
  ...
  const canSubmit =
    !!userId && meetsPolicy && validEmail && newPassword === confirm && !loading;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        // 409 = primeiro acesso já feito (segue pro login)
        if (res.status === 409) {
