...
const DOCUMENTOS_NECESSARIOS: Record<string, string[]> = {
  "secretaria-declaracao-matricula": [
    "Documento de identificação ou comprovante de RA, se solicitado pela unidade",
  ],
  ...
  "secretaria-revisao-nota": [
    "Evidência da divergência de nota/menção, quando houver",
  ],
  "secretaria-correcao-dados-siga": [
    "Documento comprobátório da alteração",
    "Documento de identificação",
  ],
  "secretaria-rematricula-fora-prazo": [
    "Comprovante de matrícula/rematrícula ou justificativa documentada",
  ],
  "secretaria-trancamento-destrancamento": [
    "Documento de identificação ou requerimento, se exigido pela unidade",
  ],
  ...
  "estagio-relatorio": [
    "Relatório de estágio assinado",
    "Avaliação do supervisor, quando aplicável",
  ],
...
export default function SolicitacaoCatalogoPage() {
  ...
  /* Busca catálogo do backend com fallback estático */
  useEffect(() => {
  ...
  /* Pré-preenche dados acadêmicos do usuário logado */
  useEffect(() => {
    if (!API) return;
  ...
  const campos = useMemo<CampoWizard[]>(() => {
    ...
    return (
      CAMPOS_ESPECIFICOS[servico.id] ??
      [campo("descricao", "Descreva sua solicitação", "textarea", true)]
    );
  }, [servico]);
  ...
  /* Upload de anexos via apiFetch (token refresh automático) */
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        ...
        planoMigracao:
          "Manter descrição serializada até o backend persistir dadosAcademicos e camposEspecificos em campos próprios.",
      };
      ...
      if (!ticketId)
        throw new Error("Solicitação criada, mas o backend não retornou o ID do ticket.");
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="rounded-xl border border-[var(--border)] bg-card p-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
          {["Serviço", "Dados acadêmicos", "Campos específicos", "Documentos", "Revisão"].map(
            (label, index) => {
      ...
      <section className="rounded-xl border border-[var(--border)] bg-card p-5">
        {/* Etapa 1 — Confirmar serviço */}
        {step === 1 && (
          <div className="space-y-4">
            <StepTitle
              title="Confirme o serviço selecionado"
              description="Confira categoria e setor provável antes de preencher os dados."
            />
        ...
        {step === 2 && (
          <div className="space-y-4">
            <StepTitle
              title="Dados acadêmicos básicos"
              description="Informe sua unidade, curso, turno, semestre, turma e RA."
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <TextInput
                label="Unidade Fatec"
                value={dadosAcademicos.unidadeFatec}
                onChange={(v) => updateAcademico("unidadeFatec", v)}
                required
                placeholder="Ex.: Fatec Zona Leste"
              />
              <TextInput
                label="Curso"
                value={dadosAcademicos.curso}
                onChange={(v) => updateAcademico("curso", v)}
                required
                placeholder="Ex.: Análise e Desenvolvimento de Sistemas"
              />
              <SelectInput
                label="Turno"
                value={dadosAcademicos.turno}
                onChange={(v) => updateAcademico("turno", v)}
                options={TURNOS}
                required
              />
              <SelectInput
                label="Semestre"
                value={dadosAcademicos.semestre}
                onChange={(v) => updateAcademico("semestre", v)}
                options={SEMESTRES}
                required
              />
              <TextInput
                label="Turma"
                value={dadosAcademicos.turma}
                onChange={(v) => updateAcademico("turma", v)}
                required
                placeholder="Ex.: ADS 3º noite"
              />
              <TextInput
                label="RA"
                value={dadosAcademicos.ra}
                onChange={(v) => updateAcademico("ra", v)}
                required
        ...
        {/* Etapa 3 — Campos específicos */}
        {step === 3 && (
          <div className="space-y-4">
            <StepTitle
              title="Campos específicos do serviço"
              description="Preencha as informações necessárias para o setor analisar sua solicitação."
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {campos.map((item) => (
                <CampoEspecifico
                  ...
                  onChange={(value) =>
                    setCamposEspecificos((c) => ({ ...c, [item.id]: value }))
                  }
                />
              ))}
            </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            <StepTitle
              ...
              description="Inclua documentos ou evidências quando forem necessários para o serviço."
            />
            {documentosNecessarios.length > 0 ? (
              ...
              <p className="rounded-lg bg-[var(--muted)] p-4 text-sm text-muted-foreground">
                Nenhum documento obrigatório mapeado para este serviço. Anexe evidências se
                ajudarem na análise.
              </p>
            )}
            ...
            <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-[var(--border)] p-8 text-center hover:bg-[var(--muted)]">
              ...
              <span className="text-xs text-muted-foreground">
                PDF, imagens, Word, Excel ou TXT · Máx. 10 MB por arquivo
              </span>
              <input
                type="file"
                multiple
                ...
                {anexos.map((file, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 text-destructive hover:opacity-70 transition"
        ...
        {step === 5 && (
          <div className="space-y-4">
            <StepTitle
              ...
              description="Confira todas as informações. Ao enviar, a solicitação será criada via POST /tickets."
            />
            <ReviewBlock
              title="Serviço"
              rows={{
            ...
            <ReviewBlock
              ...
              rows={Object.fromEntries(
                campos.map((f) => [f.label, camposEspecificos[f.id] || "Não informado"]),
              )}
            />
            <ReviewBlock
              title="Anexos"
            ...
            <label className="flex items-start gap-3 rounded-lg border border-[var(--border)] p-4 text-sm cursor-pointer">
              ...
              />
              <span>
                Confirmo que as informações são verdadeiras e autorizo o encaminhamento ao setor
                responsável.
              </span>
            </label>
          </div>
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {step < TOTAL_STEPS ? (
          <button
            type="button"
            onClick={goNext}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-primary px-4 text-sm text-primary-foreground"
          >
          ...
          <button
            type="submit"
            disabled={submitting}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-primary px-4 text-sm text-primary-foreground disabled:opacity-60"
          >
  ...
  return (
    <div>
      <h2 className="flex items-center gap-2 text-lg font-semibold">
        <CheckCircle2 className="size-5 text-primary" /> {title}
      </h2>
    ...
    return (
      <label className="space-y-1 text-sm md:col-span-2">
        ...
        <textarea
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder={campo.placeholder ?? campo.ajuda}
          rows={4}
          className="w-full rounded-lg border border-[var(--border)] bg-input p-3 focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
        />
      </label>
    );
  }
