...
export default function AdminAlunosPage() {
  ...
  function baixarModeloCSV() {
    const headers = [
      "ra", "emailEducacional", "nome", "emailPessoal",
    ...
    const exemploCompleto = [
      "654321", "maria.souza@fatec.sp.gov.br", "Maria Souza", "maria.souza@gmail.com",
      "Desenvolvimento de Software Multiplataforma", "DSM",
      "Fatec Zona Leste", "Noite", "DSM 3A Noite", "3º", "2024/1",
      "TRUE",
    ];
    const rowsData = [headers, exemploMinimo, exemploCompleto]
      .map((r) =>
        r.map((cell) => {
          const v = String(cell ?? "");
    ...
    const blob = new Blob([`﻿${rowsData}`], { type: "text/csv;charset=utf-8" });
  ...
  async function handleDeleteSelected() {
    ...
    try {
      ...
      if (failed.length) {
        alert(`Alguns registros não puderam ser excluídos (${failed.length}/${idsArray.length}).`);
      }
      setSelectedIds(new Set());
      setPage(1);
      fetchAlunos();
      ...
      {showForm ? (
        <div className="rounded-xl border border-[var(--border)] bg-card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowForm(false)} className="text-sm border px-3 h-8 rounded-md hover:bg-[var(--muted)]">← Voltar</button>
          </div>
          <FormAlunoCreate
                  ...
                  {!loading && visibleRows.map((a) => (
                    <tr key={a.id} className="border-t border-[var(--border)]">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`${ALUNO_DETAIL_PREFIX}${a.id}`} className="h-9 px-3 rounded-md hover:bg-[var(--muted)]">Ver</Link>
                      </td>
                    </tr>
                  ))}
